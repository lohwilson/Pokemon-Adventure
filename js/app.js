...
$(() => {
  ...
  function playEnemySound() {
    sound = new Audio(
      "sounds/battlesounds/" +
        enemyBattlePokemon[0].skills[random].name +
        ".mp3"
    );
    sound.play();
  }
  ...
  $pauseButton = $("<div>")
    ...
    .css("background-image", 'url("css/images/audio_on.png")');
  ...
  $professorOak1 = $("<div>")
    .addClass("professorOak1")
    .text("Professor Oak")
    .css("font-weight", "900")
    .css("color", "black");
  $oakLabDiv0.append($professorOak1);
  $professorOak2 = $("<div>")
    .addClass("professorOak2")
    .text("Professor Oak")
    .css("font-weight", "900")
    .css("color", "black");
  ...
  for (let i = 0; i < mainTownButtons.length; i++) {
    let $mainTownButton = $("<button>")
      .addClass(mainTownButtons[i])
      .text(mainTownText[i]);
  ...
  $enterBeach.hide();
  $enterDeepForest = $("<button>")
    .text("Deep Forest")
    .addClass("enterDeepForest");
  ...
  $playWithShrooms = $("<button>")
    .addClass("playWithShrooms")
    .text("Play With Shrooms");
  ...
  for (let i = 0; i < battleCommands.length; i++) {
    let $buttons = $("<button>")
      .addClass(battleCommandsButton[i])
      .text(battleCommands[i].toUpperCase());
    battleButtonsArray.push($buttons);
  }

  // END GAME PAGE DIV
  $endGamePage.append(
    $("<div>").text("Thank you for playing!").addClass("endGameText")
  );
  ...
  $createBeginJourneyButton = $("<button>")
    .attr("id", "beginJourney")
    .attr("href", "#")
    .text("Begin Journey");
  $createModalFooter.append($createBeginJourneyButton);
  $createModalFooterButton = $("<button>")
    .attr("id", "close")
    .attr("href", "#")
    .text("Close");
  ...
  $professorOak1.on("click", () => {
    ...
    alert(
      "Hello there, " +
        player.name +
        ". My name is Professor Oak. Welcome to my lab"
    );
    alert(
      "You look very excited to begin your journey, so i'll get right to it."
    );
    alert(
      "I have been a pokemon researcher for many years, and while researching about pokemon, I do have my own collection."
    );
    alert(
      "So, alas, it is your lucky day! I have prepared 3 special pokemon for you."
    );
    alert(
      "Each of them have unique talents which will be essential for your journey as a new Pokemon adventurer."
    );
    alert(
      "Please take your time to choose, but remember, the adventure is waiting for you out there."
    );
  ...
  $professorOak2.on("click", () => {
    ...
    playButtonSound();
    alert(
      "Great choice! I'm sure you will treat " +
        userPokemonList[0].name +
        " well."
    );
    alert(
      "I was supposed to introduce you to my grandson today but i'm not sure where he went."
    );
    alert("He did mention that he wanted to check out the gym in town.");
    alert(
      "But, anyway, i do have a task for you. I need you to get to town and obtain a parcel from the shopkeeper."
    );
    alert(
      "The contents of this parcel is very important to me, so please return when you have gotten it."
    );
    alert(
      "Oh, and on the way to town, you might want to test out your new Pokemon. I always hear some wild pokemon rustling near the bushes"
    );
  ...
  $gym.on("click", () => {
    ...
    if (gymClickCounter === 5) {
      ...
      alert(
        "Stranger: Time for me to show you who is the real Pokemon Master!"
      );
      $mainTown.hide();
      battle2Bgm();
      battleRival();
  ...
  $shopTalk.on("click", () => {
    ...
    alert(
      "I would love to help you but my shop a suspicious person entered my shop late last night and robbed me."
    );
    alert("Last I heard, they were spotted heading towards the forest.");
    alert(
      "Some locals mentioned that they had a secret hideout in the trees after entering the forest. You can check out that spot first."
    );
  ...
  $suspiciousTree1.on("click", () => {
    alert(
      "There's no trace of team rocket but you found some footprints leading to the beach and deeper into the forest."
    );
  ...
  $suspiciousRock1.on("click", () => {
    $suspiciousRock1.hide();
    alert(
      "The team rocket member is nowhere to be found, but there's something on the ground."
    );
  ...
  $suspiciousThing1.on("click", () => {
    $suspiciousThing1.hide();
    alert(
      "Team Rocket is nowhere to be found, but there's something on the ground."
    );
  ...
  function choosePokemonButtons() {
    ...
    for (let i = 0; i < choosePokemonArray.length; i++) {
      let $button = $("<button>")
        .addClass(choosePokemonArray[i])
        .addClass("choosePokemonButton")
        .text(
          choosePokemonArray[i].charAt(0).toUpperCase() +
            choosePokemonArray[i].slice(1)
        );
    ...
    $pikachuDiv.on("click", () => {
      ...
      if (unlockPikachu === 25) {
        ...
        $professorOak2.show();
        alert(
          "Professor Oak: Alright, alright. Since you're so persistent, I have a surprise for you!"
        );
  ...
  function checkIfTeamRocket2Defeated() {
    if (teamRocket2.pokemonList.length === 0) {
      ...
      alert(
        "Team Rocket disappeared in the distance, but something dropped down."
      );
  ...
  function battle() {
    ...
    for (let i = 0; i < 2; i++) {
      $splitDisplay = $("<div>")
        .addClass("splitDisplay")
        .attr("id", "userDisplay" + i);
    ...
    for (let i = 0; i < 2; i++) {
      $splitDisplay = $("<div>")
        .addClass("splitDisplay")
        .attr("id", "alertDisplay" + i);
      $alertBox.append($splitDisplay);
    }

    for (let i = 0; i < 12; i++) {
      let $alertButton = $("<button>")
        .addClass("alertButton" + i)
        .text("Next")
        .addClass("alertButtons")
        .hide();
    ...
    $alertButton11.on("click", () => {
      ...
      $("#alertDisplay0").text(
        enemyName + " choose " + enemyBattlePokemon[0].name
      );
      showBattleButtons();
    });
    ...
    function displayUserInfo() {
      $("#userDisplay0").text(
        "Level " + currentPokemon.level + " " + currentPokemon.name
      );
      $("#userDisplay1").text(
        "Health Points: " + userCurrentHealth + " / " + userFullHealth
      );
    ...
    function displayEnemyInfo() {
      $("#enemyDisplay0").text(
        "Level " +
          enemyBattlePokemon[0].level +
          " " +
          enemyBattlePokemon[0].name
      );
      $("#enemyDisplay1").text(
        "Health Points: " + enemyCurrentHealth + " / " + enemyFullHealth
      );
      $enemyPicture.css(
        "background-image",
        `url(${enemyBattlePokemon[0].image})`
      );
    ...
    function updateUserHealth() {
      ...
      console.log(
        currentPokemon.name + " has " + userCurrentHealth + " hitpoints left."
      );
      userPercentage = Math.round((userCurrentHealth / userFullHealth) * 100);
      $progressBar2.text(`${userPercentage}%`);
      $progressBar2.attr("style", `width: ${userPercentage}%`);
    ...
    function updateEnemyHealth() {
      ...
      enemyPercentage = Math.round(
        (enemyCurrentHealth / enemyFullHealth) * 100
      );
      $progressBar1.text(`${enemyPercentage}%`);
      $progressBar1.attr("style", `width: ${enemyPercentage}%`);
    }
    ...
    function enemyAttack() {
      ...
      userCurrentHealth =
        userCurrentHealth - enemyBattlePokemon[0].skills[random].damage;
    ...
    function checkEnemyPokemonAvailability() {
      if (enemyPokemonList.length === 0) {
        ...
        enemyBattlePokemon.push(enemyPokemonList[0]);
        $("#alertDisplay0").text(
          battleOpponent[0].name + " chose " + enemyBattlePokemon[0].name + "."
        );
        enemyPokemonList.shift();
    ...
    $enemyPicture.css(
      "background-image",
      `url(${battleOpponent[0].backgroundImg})`
    );
    ...
    for (let i = 0; i < userPokemonSkillsArray.length; i++) {
      $("#skill" + i).on("click", (event) => {
        ...
        enemyCurrentHealth =
          enemyCurrentHealth - currentPokemon.skills[i].damage;
        if (enemyCurrentHealth <= 0) {
          enemyCurrentHealth = 0;
        }
        hideSkillButtons();
        $("#alertDisplay0").text(
          currentPokemon.name + " used " + currentPokemon.skills[i].name + "."
        );
  ...
  function randomBattle() {
    ...
    $("#alertDisplay0").text(
      "A wild " + enemyBattlePokemon[0].name + " appeared."
    );
    setEnemyHealth();
    displayEnemyInfo();
